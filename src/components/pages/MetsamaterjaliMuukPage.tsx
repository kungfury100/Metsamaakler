...
export const MetsamaterjaliMuukPage: React.FC = () => {
  ...
  const faqData = [
    {
      question: "Mis vahe on raieõiguse ja metsamaterjali müügil?",
      answer: "Raieõiguse müügil ostab ettevõte õiguse metsa raiuda ja võtab kõik riskid (ilm, sortiment). Metsamaterjali müügil korraldab omanik ise raie (või tellib teenuse) ja müüb valmis palgid/paberipuu. Viimane variant on sageli tulusam, kuid nõuab rohkem organiseerimist."
    },
    {
      question: "Kuidas kujuneb puidu hind?",
      answer: "Hind sõltub puidu liigist (mänd, kuusk, kask), kvaliteedist ja diameetrist. Kõige kallim on jäme ja oksavaba palk, odavaim küttepuu. Hinda mõjutab ka maailmaturu nõudlus."
    },
    {
      question: "Kes mõõdab metsamaterjali koguse?",
      answer: "Materjal mõõdetakse tavaliselt vastuvõtja (saeveski, sadam) juures automaatliinil või virnmõõtmisega. See on kõige täpsem meetod ja alus arve esitamiseks."
    }
  ];
  ...
  const schema = {
    ...
    '@graph': [
      {
        '@type': 'Service',
        'name': 'Metsamaterjali müük',
        'description': 'Müü oma metsamaterjal parima hinnaga. Aitame transpordi, mõõtmise ja turustamisega saeveskitesse ning kokkuostjatele.',
        'provider': {
          '@type': 'LocalBusiness',
          'name': 'Metsamaakler',
      ...
      },
      {
        ...
        'itemListElement': [
          ...
          },
          {
            ...
            'position': 3,
            'name': 'Metsamaterjali müük',
            'item': 'https://metsamaakler.ee/metsamaterjali-muuk/'
          }
        ]
      },
      {
  ...
  useSeo({
    title: "Metsamaterjali müük – Palk, paberipuu ja küttepuu | Metsamaakler",
    description: "Soovid müüa metsamaterjali? Korraldame raie, transpordi ja müügi parimatele pakkujatele. Maksimaalne tulu Sinu metsast.",
    canonical: "https://metsamaakler.ee/metsamaterjali-muuk/",
    ogImage: HERO_IMAGE,
    schema: schema
  });
  ...
  return (
    <div className="min-h-screen relative bg-white font-inter selection:bg-[#347D4E] selection:text-white">
      ...
      <main className="relative z-10">
        <ServiceHero 
          image={HERO_IMAGE}
          title="Metsamaterjali müük"
          subtitle="Oled metsa üles töötanud ja soovid puitu realiseerida? Aitame leida Sinu materjalile parima ostja, korraldame transpordi ja tagame korrektse asjaajamise."
          onCtaClick={handleCtaClick}
          breadcrumbs={[
            { label: "Avaleht", href: "/" },
            { label: "Teenused", href: "/" },
            { label: "Metsamaterjali müük", href: "/metsamaterjali-muuk/" }
          ]}
        />
            ...
            <div className="lg:col-span-8">
              <article className="prose prose-lg max-w-none text-[#1a0f00]/70">
                ...
                <h2 className="text-[32px] md:text-[40px] font-bold text-[#1a0f00] mb-8 leading-[1.1] tracking-tight">
                  Maksimaalne tulu Sinu puidust
                </h2>
                
                <p className="mb-6 text-lg md:text-xl leading-snug font-normal text-[#1a0f00]/80">
                  Metsamaterjali müük on protsess, kus ülestöötatud puit (palk, paberipuu, küttepuu) sorteeritakse ja transporditakse lõpptarbijale. Erinevalt raieõiguse müügist jääb siin metsaomanikule suurem kontroll ja potentsiaalselt suurem tulu.
                </p>

                <p className="mb-10 leading-[1.5]">
                  Meie ülesanne on leida igale palgile ja notile kõige tasuvam sihtkoht. Teeme koostööd Eesti suuremate saeveskite, puidutööstuste ja katlamajadega, et tagada konkurentsivõimeline hind kogu sortimendile.
                </p>

                {/* Process Steps */}
                <h2 className="text-[28px] md:text-[32px] font-bold text-[#1a0f00] mt-16 mb-8 leading-tight">
                  Kuidas müügiprotsess toimib?
                </h2>
                ...
                <div className="space-y-6 not-prose mb-12">
                  {[
                    { 
                      icon: Scale,
                      title: "1. Sorteerimine metsas", 
                      desc: "Juba raie käigus tuleb puit õigesti järgata ja sorteerida. Palk eraldi, paberipuu eraldi. See tõstab materjali väärtust." 
                    },
                    { 
                      icon: Truck,
                      title: "2. Transport", 
                      desc: "Korraldame puiduveo metsast tööstusesse. Kasutame usaldusväärseid vedajaid ja hoolitseme veoselehtede eest." 
                    },
                    { 
                      icon: Ruler,
                      title: "3. Mõõtmine vastuvõtus", 
                      desc: "Tööstuses mõõdetakse koorem täpselt üle. Saeveskis mõõdetakse iga palki eraldi, paberipuu puhul kasutatakse sageli virnamõõtmist." 
                    },
                    { 
                      icon: Euro,
                      title: "4. Tasumine", 
                      desc: "Mõõtmisakti alusel koostatakse arve. Raha laekub Sinu kontole kokkulepitud tähtaja jooksul (tavaliselt 7-14 päeva)." 
                    }
                  ].map((step, idx) => (
                ...
                {/* Sortiment Grid */}
                <h2 className="text-[28px] md:text-[32px] font-bold text-[#1a0f00] mt-16 mb-8 leading-tight">
                  Mida me ostame ja vahendame?
                </h2>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 not-prose mb-12">
                   {[
                    { title: "Okaspuupalk", desc: "Mänd ja kuusk. Kõige väärtuslikum sortiment. Läheb saematerjaliks ja ehituspuiduks.", icon: CheckCircle2 },
                    { title: "Lehtpuupalk", desc: "Kask, haab, sanglepp. Kasutatakse vineeri, mööbli ja saematerjali tootmiseks.", icon: CheckCircle2 },
                    { title: "Paberipuu", desc: "Peenem või madalama kvaliteediga tüveosa. Läheb tselluloosi ja paberitööstusesse.", icon: FileText },
                    { title: "Küttepuu", desc: "Madala kvaliteediga puit, kõverad ja mädanikuga tüved. Kasutatakse energia tootmiseks.", icon: TrendingUp }
                   ].map((item, idx) => (
                ...
                <div className="group mb-12 p-6 rounded-xl bg-[#FFFBF0] border border-[#FDE68A] flex gap-5 not-prose">
                  ...
                  <div className="text-[#92400E] text-sm leading-relaxed">
                    <strong className="block text-[#B45309] mb-2 text-base">Kasuta metsatulu erikontot</strong>
                    Metsamaterjali müügist saadud tulu on võimalik kanda metsatulu erikontole, et lükata tulumaksu tasumist edasi ja investeerida raha tagasi metsa.
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <a href="/raieoiguse-muuk/" className="group bg-white p-6 rounded-2xl border border-gray-200 shadow-sm hover:shadow-lg hover:border-[#347D4E]/30 hover:-translate-y-1 active:scale-[0.98] transition-all duration-200 ease-out flex flex-col items-start h-full">
                      ...
                      <p className="text-sm text-gray-500 mb-6 leading-relaxed">
                        Müüd kasvavat metsa, mitte valmis materjali.
                      </p>
                    ...
                    <a href="/metsa-muuk-ja-tulumaks/" className="group bg-white p-6 rounded-2xl border border-gray-200 shadow-sm hover:shadow-lg hover:border-[#347D4E]/30 hover:-translate-y-1 active:scale-[0.98] transition-all duration-200 ease-out flex flex-col items-start h-full">
                      ...
                      <p className="text-sm text-gray-500 mb-6 leading-relaxed">
                        Kuidas deklareerida materjali müüki?
                      </p>
                      <div className="mt-auto flex items-center text-sm font-bold text-[#347D4E] group-hover:text-[#2d6e43] transition-colors duration-200">
                        Loe lähemalt <ArrowRight size={16} className="ml-2 transform group-hover:translate-x-1 transition-transform duration-200" />
                      </div>
                    ...
                    <a href="/metsamajandamiskava/" className="group bg-white p-6 rounded-2xl border border-gray-200 shadow-sm hover:shadow-lg hover:border-[#347D4E]/30 hover:-translate-y-1 active:scale-[0.98] transition-all duration-200 ease-out flex flex-col items-start h-full">
                      ...
                      <p className="text-sm text-gray-500 mb-6 leading-relaxed">
                        Hinda puidu tagavara enne raiet.
                      </p>
                      <div className="mt-auto flex items-center text-sm font-bold text-[#347D4E] group-hover:text-[#2d6e43] transition-colors duration-200">
                        Loe lähemalt <ArrowRight size={16} className="ml-2 transform group-hover:translate-x-1 transition-transform duration-200" />
                      </div>
            ...
            <div className="lg:col-span-4 space-y-8">
              ...
              <div className="sticky top-24">
                <div className="bg-gradient-to-br from-white to-[#F0F7F4] rounded-2xl p-8 border border-[#347D4E]/10 shadow-[0_20px_40px_-10px_rgba(52,125,78,0.15)] relative overflow-hidden group hover:shadow-[0_30px_60px_-15px_rgba(52,125,78,0.2)] transition-shadow duration-300">
                  ...
                  <h3 className="text-2xl font-bold text-[#1a0f00] mb-2 relative z-10">Puit virnas ootab ostjat?</h3>
                  <p className="text-[#1a0f00]/60 mb-8 relative z-10 leading-relaxed">
                    Küsi parimat hinnapakkumist oma metsamaterjalile. Organiseerime veo ja tasume kiirelt.
                  </p>
                  ...
                  <div className="space-y-4 relative z-10">
                    ...
                    <a href="mailto:info@eestimetsamaakler.ee" className="flex items-center gap-4 p-4 bg-white rounded-xl border border-black/5 shadow-sm hover:shadow-md hover:-translate-y-0.5 active:scale-[0.98] transition-all duration-200 ease-out group/item">
                      ...
                      <div>
                        <span className="text-xs font-bold text-gray-400 uppercase tracking-wider block group-hover/item:text-blue-600 transition-colors duration-200">Saada pakkumine</span>
