import React from 'react';
import { Header } from '../Header';
import { Footer } from '../Footer';
import { GlobalBackground } from '../layout/GlobalBackground';
import { ServiceHero } from '../layout/ServiceHero';
import { ContactCTASection } from '../ContactCTASection';
import { useSeo } from '../../hooks/useSeo';
import { 
  Building2, 
  Sprout, 
  Coins, 
  GraduationCap, 
  Users, 
  ExternalLink, 
  ArrowRight, 
  BookOpen,
  Phone,
  Mail,
  ChevronRight,
  Info,
  CheckCircle2,
  TreePine,
  ShieldCheck,
  Landmark
} from 'lucide-react';
import HERO_IMAGE from "figma:asset/b950c2e029e2e195de310c3ce12771f9b7c17625.png";

export const ErametsakeskusPage: React.FC = () => {
  const handleCtaClick = () => {
    const contactSection = document.getElementById('kontakt');
    if (contactSection) {
      contactSection.scrollIntoView({ behavior: 'smooth' });
    }
  };

  const faqData = [
    {
      question: "Mis on Erametsakeskuse peamine ülesanne?",
      answer: "SA Erametsakeskus (nüüd osa KIK-ist) on riigi asutatud sihtasutus, mille põhiülesanne on erametsanduse toetamine. Nad jagavad siseriiklikke ja Euroopa Liidu toetusi ning korraldavad metsaomanike nõustamist."
    },
    {
      question: "Kust ma leian infot toetuste taotlemise kohta?",
      answer: "Kõige värskem info avatud taotlusvoorude ja tingimuste kohta asub nende portaalis eramets.ee. Taotlusi esitatakse tavaliselt e-PRIA kaudu."
    },
    {
      question: "Kas Erametsakeskus ostab metsa?",
      answer: "Ei, Erametsakeskus ei tegele metsa ostu ega müügiga. Küll aga võivad nad vahendada riigi poolt ostetavate looduskaitseliste maade tehinguid või pakkuda infot selle kohta."
    },
    {
      question: "Kes võivad taotleda toetusi?",
      answer: "Toetusi saavad üldjuhul taotleda füüsilisest ja juriidilisest isikust erametsaomanikud. Mõned toetused on suunatud spetsiifiliselt metsaühistutele."
    }
  ];

  const schema = {
    '@context': 'https://schema.org',
    '@graph': [
      {
        '@type': 'Article',
        'headline': 'Erametsakeskus – Metsaomaniku teejuht',
        'description': 'Mis on SA Erametsakeskus ja kuidas see aitab metsaomanikku? Info toetuste, nõustamise ja eramets.ee portaali kohta.',
        'author': {
          '@type': 'Organization',
          'name': 'Metsamaakler'
        },
        'publisher': {
          '@type': 'Organization',
          'name': 'Metsamaakler',
          'logo': {
            '@type': 'ImageObject',
            'url': 'https://metsamaakler.ee/logo.png'
          }
        }
      },
      {
        '@type': 'BreadcrumbList',
        'itemListElement': [
          {
            '@type': 'ListItem',
            'position': 1,
            'name': 'Avaleht',
            'item': 'https://metsamaakler.ee/'
          },
          {
            '@type': 'ListItem',
            'position': 2,
            'name': 'Teenused',
            'item': 'https://metsamaakler.ee/'
          },
          {
            '@type': 'ListItem',
            'position': 3,
            'name': 'Erametsakeskus',
            'item': 'https://metsamaakler.ee/erametsakeskus/'
          }
        ]
      },
      {
        '@type': 'FAQPage',
        'mainEntity': faqData.map(item => ({
          '@type': 'Question',
          'name': item.question,
          'acceptedAnswer': {
            '@type': 'Answer',
            'text': item.answer
          }
        }))
      }
    ]
  };

  useSeo({
    title: "Erametsakeskus ja eramets.ee – Info ja toetused | Metsamaakler",
    description: "Kuidas SA Erametsakeskus saab Sulle kasulik olla? Ülevaade metsandustoetustest, tasuta nõustamisest ja eramets.ee infoportaalist.",
    canonical: "https://metsamaakler.ee/erametsakeskus/",
    ogImage: HERO_IMAGE,
    schema: schema
  });

  return (
    <div className="min-h-screen relative bg-white font-inter selection:bg-[#347D4E] selection:text-white">
      <GlobalBackground />
      <Header />
      
      <main className="relative z-10">
        <ServiceHero 
          image={HERO_IMAGE}
          title="Erametsakeskus"
          subtitle="Riiklik partner erametsaomanikule. Siitkaudu liiguvad toetused, teadmised ja nõuanded, mis aitavad Eesti metsi targalt majandada."
          onCtaClick={handleCtaClick}
          breadcrumbs={[
            { label: "Avaleht", href: "/" },
            { label: "Teenused", href: "/" },
            { label: "Erametsakeskus", href: "/erametsakeskus/" }
          ]}
        />

        <div className="max-w-[1280px] mx-auto px-6 py-12 md:py-20">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
            
            {/* MAIN CONTENT COLUMN (Left 8/12) */}
            <div className="lg:col-span-8">
              <article className="prose prose-lg max-w-none text-[#1a0f00]/70">
                
                {/* Introduction Section */}
                <h2 className="text-[32px] md:text-[40px] font-bold text-[#1a0f00] mb-8 leading-[1.1] tracking-tight">
                  Mis on SA Erametsakeskus?
                </h2>
                
                <p className="mb-6 text-lg md:text-xl leading-snug font-normal text-[#1a0f00]/80">
                  <strong>SA Erametsakeskus</strong> (nüüdseks osa Keskkonnainvesteeringute Keskusest ehk KIK-ist) on riigi poolt loodud organisatsioon, mille südameasjaks on erametsanduse arendamine Eestis.
                </p>

                <p className="mb-10 leading-[1.5]">
                  See on sild riigi ja kümnete tuhandete metsaomanike vahel. Nende eesmärk ei ole metsade majandamine kasumi eesmärgil, vaid keskkonnasõbraliku ja jätkusuutliku metsanduse soodustamine läbi rahaliste toetuste ja teadlikkuse tõstmise.
                </p>

                {/* Portal Link Box - Standardized CTA Style */}
                <div className="mb-12 not-prose">
                  <a 
                    href="https://www.eramets.ee/" 
                    target="_blank" 
                    rel="noopener noreferrer"
                    className="group flex items-center justify-between p-8 bg-[#347D4E] text-white rounded-2xl shadow-lg hover:shadow-xl hover:bg-[#2d6e43] hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.98] transition-all duration-200"
                  >
                    <div className="flex items-center gap-6">
                      <div className="bg-white/20 w-16 h-16 rounded-full flex items-center justify-center backdrop-blur-sm shadow-inner group-hover:scale-110 transition-transform duration-200">
                        <ExternalLink size={32} className="text-white" />
                      </div>
                      <div>
                        <h4 className="font-bold text-xl mb-1 text-white">Külasta portaali eramets.ee</h4>
                        <p className="text-white/80 text-base m-0 font-medium">Kõige värskem info toetuste kohta</p>
                      </div>
                    </div>
                    <div className="bg-white/10 p-3 rounded-full group-hover:bg-white/20 transition-colors">
                      <ArrowRight size={24} className="text-white group-hover:translate-x-1 transition-transform duration-200" />
                    </div>
                  </a>
                </div>

                <h2 className="text-[28px] md:text-[32px] font-bold text-[#1a0f00] mt-16 mb-8 leading-tight">
                  Kuidas Erametsakeskus Sind aitab?
                </h2>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 not-prose mb-12">
                   {[
                    { 
                      title: "Metsandustoetused", 
                      desc: "Kõige olulisem funktsioon. Menetletakse toetusi metsa uuendamiseks, hooldamiseks ja looduskaitseliste piirangute kompenseerimiseks.",
                      icon: Coins
                    },
                    { 
                      title: "Nõustamine ja koolitused", 
                      desc: "Kui jääte hätta, suunatakse teid tunnustatud konsulentide juurde. Erametsaomanikele on teatud mahus nõustamine tasuta.",
                      icon: GraduationCap
                    },
                    { 
                      title: "Metsaühistute tugi", 
                      desc: "Toetatakse kohalikke metsaühistuid, et väikemetsaomanikud saaksid parematel tingimustel oma metsa majandada.",
                      icon: Users
                    },
                    { 
                      title: "Infoportaal eramets.ee", 
                      desc: "Teadmistepank, kust leiate puiduhinna statistikat, seadusemuudatusi ja toetuste voorude tähtaegu.",
                      icon: BookOpen
                    }
                   ].map((item, idx) => (
                    <div key={idx} className="bg-white p-6 rounded-xl border border-gray-100 hover:shadow-md hover:border-[#347D4E]/30 transition-all duration-200 ease-out group flex flex-col h-full">
                      <div className="mb-4 text-[#347D4E] group-hover:scale-110 transition-transform duration-200">
                        <item.icon size={28} strokeWidth={1.5} />
                      </div>
                      <h4 className="font-bold text-[#1a0f00] text-lg mb-2">{item.title}</h4>
                      <p className="text-sm text-[#1a0f00]/60 m-0 leading-relaxed flex-grow">{item.desc}</p>
                    </div>
                   ))}
                </div>

                <h2 className="text-[28px] md:text-[32px] font-bold text-[#1a0f00] mt-16 mb-8 leading-tight">
                  Olulisemad toetused lühidalt
                </h2>
                <p className="mb-6">
                  Metsaomanikuna on kasulik teada, millal ja mida küsida saab. Siin on peamised toetusliigid:
                </p>
                
                {/* Support Types List - Enhanced Visuals */}
                <div className="space-y-4 not-prose mb-12">
                  <div className="flex gap-5 p-6 bg-white rounded-xl border border-gray-100 hover:border-[#347D4E]/30 hover:shadow-sm transition-all duration-200 group">
                    <div className="shrink-0 mt-1 p-3 bg-green-50 rounded-full text-[#347D4E] group-hover:bg-[#347D4E] group-hover:text-white transition-colors duration-200">
                      <Sprout size={24} strokeWidth={1.5} />
                    </div>
                    <div>
                      <h4 className="font-bold text-[#1a0f00] text-lg mb-2">Metsa uuendamise toetus</h4>
                      <p className="text-sm text-[#1a0f00]/70 m-0 leading-relaxed">
                        Kompenseeritakse osaliselt istikute soetamist ja istutamistööd. Eesmärk on tagada, et pärast raiet kasvaks asemele kvaliteetne uus mets.
                      </p>
                    </div>
                  </div>

                  <div className="flex gap-5 p-6 bg-white rounded-xl border border-gray-100 hover:border-[#347D4E]/30 hover:shadow-sm transition-all duration-200 group">
                    <div className="shrink-0 mt-1 p-3 bg-green-50 rounded-full text-[#347D4E] group-hover:bg-[#347D4E] group-hover:text-white transition-colors duration-200">
                      <ShieldCheck size={24} strokeWidth={1.5} />
                    </div>
                    <div>
                      <h4 className="font-bold text-[#1a0f00] text-lg mb-2">Natura 2000 toetus</h4>
                      <p className="text-sm text-[#1a0f00]/70 m-0 leading-relaxed">
                        Iga-aastane kompensatsioon omanikele, kelle metsas on looduskaitselised piirangud (Natura alal), mis takistavad majandustegevust ja vähendavad tulu.
                      </p>
                    </div>
                  </div>

                  <div className="flex gap-5 p-6 bg-white rounded-xl border border-gray-100 hover:border-[#347D4E]/30 hover:shadow-sm transition-all duration-200 group">
                    <div className="shrink-0 mt-1 p-3 bg-green-50 rounded-full text-[#347D4E] group-hover:bg-[#347D4E] group-hover:text-white transition-colors duration-200">
                      <Landmark size={24} strokeWidth={1.5} />
                    </div>
                    <div>
                      <h4 className="font-bold text-[#1a0f00] text-lg mb-2">Pärandkultuuri toetus</h4>
                      <p className="text-sm text-[#1a0f00]/70 m-0 leading-relaxed">
                        Aitab säilitada ja eksponeerida metsas asuvaid kultuuriväärtusi, näiteks vanu kiviaedu, talukohti või metsavendade punkreid.
                      </p>
                    </div>
                  </div>
                </div>

                {/* Location/Contact Info Block - Styled as Warning/Info */}
                <div className="bg-[#FFFBF0] p-8 rounded-xl border border-[#FDE68A] my-12 not-prose flex gap-5 items-start">
                  <div className="shrink-0 mt-1 text-[#F59E0B]">
                    <Info size={28} strokeWidth={1.5} />
                  </div>
                  <div>
                    <h3 className="font-bold text-[#92400E] text-lg mb-2">Kus asub Erametsakeskus?</h3>
                    <p className="text-[#92400E]/80 m-0 text-sm leading-relaxed">
                      Füüsiliselt asuvad nad Tallinnas, kuid tänapäeval toimub peamine suhtlus elektrooniliselt (e-posti ja e-PRIA kaudu). Piirkondlikku tuge pakuvad kohalikud metsaühistud.
                    </p>
                  </div>
                </div>

                <h2 className="text-[28px] md:text-[32px] font-bold text-[#1a0f00] mt-16 mb-8 leading-tight">
                  Korduma kippuvad küsimused
                </h2>
                <div className="space-y-4 not-prose">
                  {faqData.map((faq, i) => (
                    <div key={i} className="group bg-white rounded-xl border border-gray-100 p-6 hover:border-[#347D4E]/30 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 ease-out cursor-pointer active:scale-[0.99]">
                      <h4 className="font-bold text-lg text-[#1a0f00] mb-3 flex items-start justify-between gap-4 group-hover:text-[#347D4E] transition-colors duration-200">
                        {faq.question}
                        <div className="mt-1 text-gray-300 group-hover:text-[#347D4E] transition-colors duration-200">
                           <ChevronRight size={18} className="group-hover:rotate-90 transition-transform duration-200" />
                        </div>
                      </h4>
                      <p className="text-[#1a0f00]/70 leading-relaxed mb-0 text-[15px] max-h-0 overflow-hidden group-hover:max-h-96 transition-all duration-300 ease-out opacity-0 group-hover:opacity-100">
                        {faq.answer}
                      </p>
                    </div>
                  ))}
                </div>
                
                {/* Related Content - Clean Icons */}
                <div className="mt-20 pt-12 border-t border-gray-100 not-prose">
                  <h3 className="text-[24px] font-bold text-[#1a0f00] mb-8">Vaadake ka:</h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {/* Card 1 */}
                    <a href="/metsa-uuendamise-toetus/" className="group bg-white p-6 rounded-2xl border border-gray-200 shadow-sm hover:shadow-lg hover:border-[#347D4E]/30 hover:-translate-y-1 active:scale-[0.98] transition-all duration-200 ease-out flex flex-col items-start h-full">
                      <div className="mb-6 text-[#347D4E] group-hover:scale-105 transition-transform duration-200">
                        <Sprout size={32} strokeWidth={1.5} />
                      </div>
                      <h4 className="font-bold text-[#1a0f00] text-lg mb-2 group-hover:text-[#347D4E] transition-colors duration-200">
                        Uuendamise toetus
                      </h4>
                      <p className="text-sm text-gray-500 mb-6 leading-relaxed">
                        Toetus uue metsa rajamiseks.
                      </p>
                      <div className="mt-auto flex items-center text-sm font-bold text-[#347D4E] group-hover:text-[#2d6e43] transition-colors duration-200">
                        Loe lähemalt <ArrowRight size={16} className="ml-2 transform group-hover:translate-x-1 transition-transform duration-200" />
                      </div>
                    </a>

                    {/* Card 2 */}
                    <a href="/metsamajandamiskava/" className="group bg-white p-6 rounded-2xl border border-gray-200 shadow-sm hover:shadow-lg hover:border-[#347D4E]/30 hover:-translate-y-1 active:scale-[0.98] transition-all duration-200 ease-out flex flex-col items-start h-full">
                      <div className="mb-6 text-[#347D4E] group-hover:scale-105 transition-transform duration-200">
                        <TreePine size={32} strokeWidth={1.5} />
                      </div>
                      <h4 className="font-bold text-[#1a0f00] text-lg mb-2 group-hover:text-[#347D4E] transition-colors duration-200">
                        Metsakava
                      </h4>
                      <p className="text-sm text-gray-500 mb-6 leading-relaxed">
                        Vajalik toetuste taotlemiseks.
                      </p>
                      <div className="mt-auto flex items-center text-sm font-bold text-[#347D4E] group-hover:text-[#2d6e43] transition-colors duration-200">
                        Loe lähemalt <ArrowRight size={16} className="ml-2 transform group-hover:translate-x-1 transition-transform duration-200" />
                      </div>
                    </a>

                    {/* Card 3 */}
                    <a href="/natura-2000-alade-piirangud/" className="group bg-white p-6 rounded-2xl border border-gray-200 shadow-sm hover:shadow-lg hover:border-[#347D4E]/30 hover:-translate-y-1 active:scale-[0.98] transition-all duration-200 ease-out flex flex-col items-start h-full">
                      <div className="mb-6 text-[#347D4E] group-hover:scale-105 transition-transform duration-200">
                        <Building2 size={32} strokeWidth={1.5} />
                      </div>
                      <h4 className="font-bold text-[#1a0f00] text-lg mb-2 group-hover:text-[#347D4E] transition-colors duration-200">
                        Natura 2000
                      </h4>
                      <p className="text-sm text-gray-500 mb-6 leading-relaxed">
                        Looduskaitse toetused.
                      </p>
                      <div className="mt-auto flex items-center text-sm font-bold text-[#347D4E] group-hover:text-[#2d6e43] transition-colors duration-200">
                        Loe lähemalt <ArrowRight size={16} className="ml-2 transform group-hover:translate-x-1 transition-transform duration-200" />
                      </div>
                    </a>
                  </div>
                </div>

              </article>
            </div>

            {/* SIDEBAR COLUMN (Right 4/12) - STICKY */}
            <div className="lg:col-span-4 space-y-8">
              
              {/* Sticky Contact Card */}
              <div className="sticky top-24">
                <div className="bg-gradient-to-br from-white to-[#F0F7F4] rounded-2xl p-8 border border-[#347D4E]/10 shadow-[0_20px_40px_-10px_rgba(52,125,78,0.15)] relative overflow-hidden group hover:shadow-[0_30px_60px_-15px_rgba(52,125,78,0.2)] transition-shadow duration-300">
                  <div className="absolute -top-24 -right-24 w-48 h-48 bg-[#347D4E]/5 rounded-full blur-3xl group-hover:bg-[#347D4E]/10 transition-colors duration-500" />

                  <h3 className="text-2xl font-bold text-[#1a0f00] mb-2 relative z-10">Bürokraatia ajab segadusse?</h3>
                  <p className="text-[#1a0f00]/60 mb-8 relative z-10 leading-relaxed">
                    Aitame leida õiged toetused ja hoolitseme, et ükski võimalus ei jääks kasutamata. Küsi tasuta nõu.
                  </p>

                  <div className="space-y-4 relative z-10">
                    <a href="tel:+3725551234" className="flex items-center gap-4 p-4 bg-white rounded-xl border border-black/5 shadow-sm hover:shadow-md hover:-translate-y-0.5 active:scale-[0.98] transition-all duration-200 ease-out group/item">
                      <div className="text-[#347D4E] group-hover/item:scale-105 transition-transform duration-200">
                        <Phone size={24} strokeWidth={1.5} />
                      </div>
                      <div>
                        <span className="text-xs font-bold text-gray-400 uppercase tracking-wider block group-hover/item:text-[#347D4E] transition-colors duration-200">Helista meile</span>
                        <span className="font-bold text-[#1a0f00] transition-colors">+372 555 1234</span>
                      </div>
                    </a>

                    <a href="mailto:info@metsamaakler.ee" className="flex items-center gap-4 p-4 bg-white rounded-xl border border-black/5 shadow-sm hover:shadow-md hover:-translate-y-0.5 active:scale-[0.98] transition-all duration-200 ease-out group/item">
                      <div className="text-blue-600 group-hover/item:scale-105 transition-transform duration-200">
                        <Mail size={24} strokeWidth={1.5} />
                      </div>
                      <div>
                        <span className="text-xs font-bold text-gray-400 uppercase tracking-wider block group-hover/item:text-blue-600 transition-colors duration-200">Kirjuta meile</span>
                        <span className="font-bold text-[#1a0f00] transition-colors">info@metsamaakler.ee</span>
                      </div>
                    </a>
                  </div>

                  <button 
                    onClick={handleCtaClick}
                    className="w-full mt-8 bg-[#347D4E] hover:bg-[#2d6e43] text-white py-4 rounded-xl font-bold text-lg shadow-lg hover:shadow-xl hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.95] transition-all duration-200 ease-out flex items-center justify-center gap-2 group/btn relative overflow-hidden"
                  >
                    <span className="relative z-10 flex items-center gap-2">
                      Küsi nõu
                      <ArrowRight size={20} className="group-hover/btn:translate-x-1 transition-transform duration-200" />
                    </span>
                    <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent translate-x-[-100%] group-hover/btn:translate-x-[100%] transition-transform duration-700 ease-in-out" />
                  </button>
                </div>
              </div>

            </div>
          </div>
        </div>

        <ContactCTASection showCTA={false} transparentBg={true} />
      </main>

      <Footer />
    </div>
  );
};
